-- Corrective Migration: Fix PostgreSQL 42702 (ambiguous_column) in zeya_persist_hypothesis
--
-- Issue: RETURN QUERY statements return unaliased columns that don't match RETURNS TABLE definition
-- Error: ambiguous_column (42702) on all 7 RPC calls during Preview test
-- Impact: zeya_persist_hypothesis cannot execute; orchestration persistence fails
-- Fix: Explicitly alias SELECT columns to match RETURNS TABLE output names

-- Recreate function with identical 12-parameter signature and corrected RETURN QUERY statements
...
BEGIN
  ...
  IF p_request_trace_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      -- FIXED: Explicitly alias SELECT columns to match RETURNS TABLE output names
      RETURN QUERY
      SELECT
...
-- Explicit ACL: service-role only (signature unchanged from 20260808000001)
