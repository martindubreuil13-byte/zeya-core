...
CREATE OR REPLACE FUNCTION public.zeya_prepare_operating_mission_v2(p_owner_id uuid,p_mission_id uuid,p_prospect_context jsonb,p_prospect_source_fingerprint text)
...
BEGIN
  ...
  SELECT mission.* INTO v_mission FROM public.operating_missions AS mission WHERE mission.id=p_mission_id AND mission.owner_id=p_owner_id FOR UPDATE;
  IF v_mission.id IS NULL THEN RAISE EXCEPTION USING ERRCODE='PZ404',MESSAGE='mission not found'; END IF;
  SELECT representation.* INTO v_rep FROM public.business_representations AS representation WHERE representation.id=v_mission.business_representation_id AND representation.user_id=p_owner_id FOR SHARE;
  SELECT lead.* INTO v_lead FROM public.mission_leads AS lead WHERE lead.id=v_mission.lead_id AND lead.business_representation_id=v_rep.id FOR SHARE;
  SELECT version.* INTO v_version FROM public.representation_versions AS version WHERE version.id=v_mission.representation_version_id AND version.business_representation_id=v_rep.id;
  SELECT package.* INTO v_outcome FROM public.direct_hire_formation_outcome_packages AS package WHERE package.id=v_mission.mandate_outcome_package_id AND package.owner_id=p_owner_id AND package.business_representation_id=v_rep.id;
  ...
  SELECT context_row.id,context_row.context,context_row.owner_id,context_row.business_representation_id,context_row.representation_version_id,context_row.mandate_outcome_package_id
  ...
  FROM public.mission_execution_contexts AS context_row WHERE context_row.mission_id=v_mission.id;
  ...
  UPDATE public.operating_missions AS mission SET status='ready',updated_at=pg_catalog.now() WHERE mission.id=v_mission.id AND mission.status='draft';
