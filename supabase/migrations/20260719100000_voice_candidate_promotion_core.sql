...
CREATE FUNCTION public.zeya_promote_voice_conversation_candidate_internal(p_actor_user_id uuid,p_candidate_id uuid,p_target_type public.conversation_candidate_promotion_target,p_request_key uuid,p_confirmed_content jsonb,p_reason text,p_related_element_id uuid,p_evidence_source_type public.evidence_source_type)
...
BEGIN
 IF p_actor_user_id IS NULL THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='promotion not authorized'; END IF;
 ...
 SELECT * INTO c FROM public.voice_conversation_candidates WHERE id=p_candidate_id AND tenant_user_id=p_actor_user_id FOR UPDATE;
 ...
 VALUES(c.id,c.conversation_output_id,o.voice_context_id,c.tenant_user_id,c.business_id,c.business_representation_id,c.canonical_version_id,p_actor_user_id,'accepted_for_promotion',reason,c.transcript_trust_level,c.extraction_schema_version,p_request_key) RETURNING id INTO v_decision_id;
 ...
 VALUES(v_decision_id,c.id,c.conversation_output_id,o.voice_context_id,c.tenant_user_id,c.business_id,c.business_representation_id,c.canonical_version_id,p_actor_user_id,p_target_type,p_request_key,payload,payload_hash,reason,p_related_element_id,p_evidence_source_type,CASE WHEN p_target_type='evidence' THEN v_evidence_id END,CASE WHEN p_target_type='observation' THEN v_observation_id END,CASE WHEN p_target_type='representation_proposal' THEN v_proposal_id END,c.content,confirmed) RETURNING id INTO v_promotion_id;
...
CREATE OR REPLACE FUNCTION public.zeya_promote_voice_conversation_candidate(p_candidate_id uuid,p_target_type public.conversation_candidate_promotion_target,p_request_key uuid,p_confirmed_content jsonb,p_reason text DEFAULT NULL::text,p_related_element_id uuid DEFAULT NULL::uuid,p_evidence_source_type public.evidence_source_type DEFAULT 'conversation'::public.evidence_source_type)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
BEGIN
 ...
 RETURN public.zeya_promote_voice_conversation_candidate_internal(auth.uid(),p_candidate_id,p_target_type,p_request_key,p_confirmed_content,p_reason,p_related_element_id,p_evidence_source_type);
END; $$;

ALTER FUNCTION public.zeya_promote_voice_conversation_candidate_internal(uuid,uuid,public.conversation_candidate_promotion_target,uuid,jsonb,text,uuid,public.evidence_source_type) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.zeya_promote_voice_conversation_candidate_internal(uuid,uuid,public.conversation_candidate_promotion_target,uuid,jsonb,text,uuid,public.evidence_source_type) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.zeya_promote_voice_conversation_candidate_internal(uuid,uuid,public.conversation_candidate_promotion_target,uuid,jsonb,text,uuid,public.evidence_source_type) TO postgres;
ALTER FUNCTION public.zeya_promote_voice_conversation_candidate(uuid,public.conversation_candidate_promotion_target,uuid,jsonb,text,uuid,public.evidence_source_type) OWNER TO postgres;
