-- Corrective Migration: Add p_epistemic_state parameter to zeya_persist_hypothesis RPC
--
-- Issue: Function signature missing p_epistemic_state parameter that function body uses
-- Impact: Function is non-executable; orchestration cannot call it
-- Fix: Add p_epistemic_state as 4th parameter; drop old 11-param overload to avoid ambiguity

-- Drop old ACL for 11-parameter version
...
-- Drop old 11-parameter function to prevent ambiguity with new 12-parameter version
DROP FUNCTION IF EXISTS public.zeya_persist_hypothesis(UUID, UUID, VARCHAR, TEXT, VARCHAR, VARCHAR, TEXT, UUID[], TIMESTAMP WITH TIME ZONE, VARCHAR, TEXT);

-- Recreate function with correct 12-parameter signature (adds p_epistemic_state as 4th param)
...
-- Explicit ACL: service-role only (updated signature with 12 parameters)
