 ...
 * With proper prospect memory projection via P2.9C
 */
...
async function run() {
  ...
  try {
    ...
    // STEP 2: Build prospect context from observations (P2.9C path)
    console.log('STEP 2: Building prospect context from observations...\n');
    ...
    if (!obsRes.data || obsRes.data.length === 0) {
      throw new Error('No prospect observations found');
    }

    const observations = obsRes.data;
    console.log(`✓ Retrieved ${observations.length} prospect observations\n`);

    // Build prospectContextV1
    const prospectContext = {
      schemaVersion: 'prospect-context-v1',
      leadId: SYNTHETIC_LEAD_ID,
      currentFacts: observations.map((obs) => ({
        slot: obs.slot || 'unclassified',
        summary: obs.observation_summary || obs.content || '',
      })),
      obligations: [],
    ...
    const sourceFingerprint = crypto.randomBytes(32).toString('hex');
    console.log(`✓ Prospect context built with ${observations.length} facts\n`);
    ...
    // STEP 7: Verify prospect memory is intact
    console.log('STEP 7: Verifying prospect memory continuity...\n');

    const observationsVerifyRes = await db
      .from('prospect_observations')
      .select('id')
      .eq('lead_id', SYNTHETIC_LEAD_ID)
      .eq('owner_id', QA_OWNER_ID);

    const obsCount = observationsVerifyRes.data?.length || 0;

    const relationsVerifyRes = await db
      .from('prospect_observation_relations')
      .select('id')
      .eq('lead_id', SYNTHETIC_LEAD_ID)
      .eq('owner_id', QA_OWNER_ID);

    const relCount = relationsVerifyRes.data?.length || 0;
