#!/usr/bin/env npx tsx
/**
 * P2.10D — CREATE FRESH CHAIN (NEW MISSION + CONTEXT + V3 DISPATCH)
 * 
 * Creates completely fresh artifacts via authoritative P2.9D RPC path.
 * Old contaminated artifacts preserved as historical evidence.
 */
...
async function run() {
  ...
  try {
    ...
    // STEP 2: Prepare mission to create execution context
    console.log('STEP 2: Preparing mission (creates execution context)...\n');

    const prepareOpId = generateUuid();
    const prepareRes = await db.rpc('zeya_prepare_operating_mission_v2', {
      p_owner_id: QA_OWNER_ID,
      p_mission_id: newMissionId,
      p_operation_id: prepareOpId,
    });
    ...
    const newContextId = prepareData.execution_context_id;
    console.log(`✓ Fresh execution context created: ${newContextId}`);
    console.log(`✓ Mission status: ${prepareData.status}\n`);

    // STEP 3: Create dispatch via P2.9D RPC with full governance parameters
    console.log('STEP 3: Creating dispatch via P2.9D RPC (with full commercial conversation semantics)...\n');
    ...
    // STEP 4: Verify brief has correct contract
    console.log('STEP 4: Verifying worker brief contract...\n');
    ...
    // STEP 6: Verify prospect memory is intact
    console.log('STEP 6: Verifying prospect memory continuity...\n');
    ...
    const obsCount = observationsRes.data?.length || 0;
    ...
    const relCount = relationsRes.data?.length || 0;

    console.log(`✓ Prospect observations: ${obsCount} (expected: 8)`);
    console.log(`✓ Prospect relations: ${relCount} (expected: 0)\n`);
    ...
    console.log(`Worker Identity:         ${briefPayload.worker?.spokenName || 'Veya'}`);
    console.log(`Conversation Policy:     ${briefPayload.conversationPolicy?.schemaVersion}`);
    console.log(`Conversation Mode:       ${briefPayload.conversationPolicy?.mode}`);
    console.log(`Capabilities:            ${briefPayload.capabilities?.schemaVersion}`);
    console.log(`Prospect Context:        ${briefPayload.prospect?.context?.schemaVersion}`);
    console.log(`Opening Contract:        ${briefPayload.openingContract?.schemaVersion}`);
    console.log(`Opening Deterministic:   ${!!briefPayload.openingContract?.introductionAlreadySpoken}\n`);

    console.log('EXECUTION READINESS:\n');
    console.log(`Execution Attempts:      0 (expected: 0)`);
    console.log(`Provider Calls:          0 (expected: 0)`);
    console.log(`Prospect Memory Intact:  obs=${obsCount}, rel=${relCount}`);
