...
async function main(){
  ...
  try{
    ...
    const correlated=await admin.from("public_experience_sessions").select("veya_voice_context_id,state,phone_hash,dispatch_id").eq("id",session.data.id).single();assert(correlated.data?.veya_voice_context_id&&correlated.data.state==="call_dispatched"&&correlated.data.phone_hash&&!JSON.stringify(correlated.data).includes(phone),"Veya correlation or phone privacy failed");registry.registerVoiceLineage(correlated.data.veya_voice_context_id,representationId);const persistedBrief=await admin.from("worker_briefs").select("target_phone,dynamic_variables").eq("mission_id",correlated.data.dispatch_id).single();assert(!persistedBrief.error&&persistedBrief.data?.target_phone===null&&!JSON.stringify(persistedBrief.data.dynamic_variables).includes(phone),"Public Experience WorkerBrief persisted plaintext phone");const persistedMapping=await admin.from("brief_conversation_mappings").select("*").eq("mission_id",correlated.data.dispatch_id).single();assert(!persistedMapping.error&&!JSON.stringify(persistedMapping.data).includes(phone),"Public Experience mapping persisted plaintext phone");const veyaLineage=await admin.from("voice_representation_lineage").select("business_representation_id,canonical_version_id,conversation_id").eq("voice_context_id",correlated.data.veya_voice_context_id).single();assert(veyaLineage.data?.business_representation_id===representationId&&veyaLineage.data.canonical_version_id===versionId&&correlated.data.veya_voice_context_id!==session.data.zeya_voice_context_id,"Veya lineage isolation mismatch");
