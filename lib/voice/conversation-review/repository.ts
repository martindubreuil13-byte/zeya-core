...
export async function listReviewConversations(db: SupabaseClient, businessId: string): Promise<ReviewConversation[]> {
  ...
  const [{ data: candidates, error: candidateError }, { data: decisions, error: decisionError }, { data: promotions, error: promotionError }, { data: elements, error: elementError }] = await Promise.all([
  ...
  if (candidateError || decisionError || promotionError || elementError) throw candidateError ?? decisionError ?? promotionError ?? elementError;
  return outputs.map((output) => ({
    ...
    candidates: (candidates ?? []).filter((candidate) => candidate.conversation_output_id === output.id).map((candidate) => {
      ...
      return {
        id: candidate.id, conversationOutputId: candidate.conversation_output_id, candidateType: candidate.candidate_type,
