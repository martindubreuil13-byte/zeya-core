...
DO $$
BEGIN
 IF to_regclass('public.conversation_candidate_canonical_promotions') IS NOT NULL THEN
  RAISE EXCEPTION 'Phase 5B canonical bridge dependency exists; refuse rollback';
 END IF;
END; $$;
...
DROP FUNCTION public.zeya_promote_voice_conversation_candidate_internal(uuid,uuid,public.conversation_candidate_promotion_target,uuid,jsonb,text,uuid,public.evidence_source_type);
COMMIT;
