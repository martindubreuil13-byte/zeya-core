-- Rollback restores the exact pre-correction RPC, including its known runtime
-- ambiguity. Use only to undo the correction while preserving the RPC object.
BEGIN;
  ...
  IF NOT EXISTS (
    ...
    WHERE p.oid = pg_catalog.to_regprocedure(
      ...
      AND pg_catalog.regexp_replace(p.prosrc, '[[:space:]]+', '', 'g')
            ~* 'max\(version_row\.version_number\)'
  ) THEN
    RAISE EXCEPTION 'corrected atomic canonical Version RPC not found; refusing rollback';
  END IF;
END;
$$;
...
BEGIN
  ...
  ) RETURNING id,created_at INTO v_new_version_id,v_new_version_created_at;
