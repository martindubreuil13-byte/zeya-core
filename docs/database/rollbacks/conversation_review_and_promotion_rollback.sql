...
-- MANUAL EMERGENCY ROLLBACK ONLY. Restores the checked-in Phase 2 purge
-- definition exactly, then removes only Phase 3 review-layer objects.
...
DROP FUNCTION IF EXISTS public.zeya_promote_voice_conversation_candidate(uuid, public.conversation_candidate_promotion_target, uuid, jsonb, text, uuid, public.evidence_source_type);
DROP FUNCTION IF EXISTS public.zeya_review_voice_conversation_candidate(uuid, public.conversation_candidate_decision_type, uuid, text);
DROP TRIGGER IF EXISTS zeya_conversation_promotion_immutability ON public.conversation_candidate_promotions;
DROP TRIGGER IF EXISTS zeya_conversation_review_immutability ON public.conversation_candidate_review_decisions;
DROP TABLE IF EXISTS public.conversation_candidate_promotions;
DROP TABLE IF EXISTS public.conversation_candidate_review_decisions;
DROP FUNCTION IF EXISTS public.zeya_enforce_conversation_review_immutability();
DROP INDEX IF EXISTS public.voice_outputs_review_identity_idx;
DROP INDEX IF EXISTS public.voice_candidates_review_identity_idx;
DROP TYPE IF EXISTS public.conversation_candidate_promotion_target;
DROP TYPE IF EXISTS public.conversation_candidate_decision_type;
