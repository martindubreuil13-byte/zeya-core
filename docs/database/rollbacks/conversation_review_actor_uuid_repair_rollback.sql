-- Emergency rollback only: restores the previously deployed actor text definition.
-- Promotion will again fail against UUID actor columns; use only to reverse this repair.
BEGIN;
...
DECLARE c public.voice_conversation_candidates%ROWTYPE; o public.voice_conversation_outputs%ROWTYPE;
 ...
 statement text; actor text; reason text; confirmed jsonb; payload jsonb; payload_hash text; allowed boolean:=false;
