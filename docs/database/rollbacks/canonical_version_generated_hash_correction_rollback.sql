-- Emergency structural rollback only. Restores the immediately previous RPC,
-- which explicitly writes a generated column and is not operationally healthy.
BEGIN;
  ...
  IF NOT EXISTS (
    ...
    WHERE p.oid = pg_catalog.to_regprocedure(
      ...
      AND pg_catalog.regexp_replace(p.prosrc, '[[:space:]]+', '', 'g')
            ~* 'insertintopublic\.representation_versionsasinserted_version\(business_representation_id,previous_version_id,source_proposal_id,source_approval_id,element_values,version_number,overall_confidence_score,created_by_actor\)values\('
  ) THEN
    RAISE EXCEPTION 'generated-hash correction not found; refusing rollback';
  END IF;
END;
$$;
...
BEGIN
  ...
  INSERT INTO public.representation_versions AS inserted_version(
    ...
    source_approval_id,element_values,version_number,overall_confidence_score,
    created_by_actor,content_hash
  ) VALUES(
  ...
  INSERT INTO public.audit_events(
    ...
    v_new_version_id,p_actor_user_id,
    jsonb_build_object(
...
ALTER FUNCTION public.zeya_create_canonical_version_atomic(
  UUID,UUID,UUID,JSONB,SMALLINT,UUID,UUID
) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.zeya_create_canonical_version_atomic(
  UUID,UUID,UUID,JSONB,SMALLINT,UUID,UUID
) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.zeya_create_canonical_version_atomic(
  UUID,UUID,UUID,JSONB,SMALLINT,UUID,UUID
) TO service_role;
