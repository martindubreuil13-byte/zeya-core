...
export async function POST(request:NextRequest,{params}:{params:Promise<{missionId:string}>}){
  ...
  if(result.error)return NextResponse.json({success:false,error:result.error.code==='PZ404'?'mission_not_found':'mission_stale_or_not_preparable'},{status:result.error.code==='PZ404'?404:409});
