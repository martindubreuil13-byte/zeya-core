...
export function ConversationReviewPanel({ businessId, session }: { businessId: string; session: Session | null }) {
  ...
  const [canonicalizationEnabled, setCanonicalizationEnabled] = useState(false);
  ...
  async function load() {
    ...
    try {
      ...
      const body = await response.json() as { success: boolean; data?: ReviewConversation[]; error?: string; canonicalizationEnabled?: boolean };
      if (!response.ok || !body.data) throw new Error(body.error ?? "Unable to load conversations");
      const conversations = body.data;
      setCanonicalizationEnabled(body.canonicalizationEnabled === true);
  ...
  async function act(candidateId: string, payload: Record<string, unknown>, requestKey = crypto.randomUUID()): Promise<Record<string, unknown> | null> {
    if (!session?.access_token) return null;
    setBusyCandidate(candidateId); setError(null); setSuccess(null);
    try {
      const response = await fetch("/api/voice/conversation-review", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ candidateId, requestKey, ...payload }) });
      const body = await response.json() as { error?: string; message?: string; data?: Record<string, unknown> };
      if (!response.ok) throw new Error(body.message ?? body.error ?? "Review action failed");
      await load();
      return body.data ?? null;
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Review action failed"); return null; }
  ...
  return (
    <section aria-label="Conversation review" className="w-full border-t border-zeya-graphite/14 py-6">
      ...
      {success && <div role="status" className="mb-4 rounded-lg border border-zeya-champagne/25 p-3 text-xs text-zeya-champagne/80">{success}</div>}
      ...
      {conversations.length === 0 ? <p className="py-8 text-center text-sm text-zeya-hush/45">No captured conversations are ready for review.</p> : (
        <div className="grid gap-5 md:grid-cols-[13rem_1fr]">
          ...
          {selected && <div><div className="mb-4 rounded-xl border border-zeya-graphite/20 p-4"><div className="flex justify-between gap-4"><span className="text-sm text-zeya-ivory/80">Conversation briefing</span><span className="text-xs text-zeya-hush/50">Canonical context {selected.canonicalVersionId.slice(0, 8)}</span></div><p className="mt-2 text-xs leading-relaxed text-zeya-hush/58">{trustLabel[selected.trustLevel]}. Review is required before anything enters the Representation pipeline.</p><details className="mt-3"><summary className="cursor-pointer text-xs text-zeya-champagne/65">View safe transcript</summary><div className="mt-3 max-h-52 space-y-2 overflow-auto">{selected.transcript.map((turn, index) => <p key={index} className="text-xs text-zeya-hush/60"><span className="text-zeya-ivory/65">{turn.role}:</span> {turn.text}</p>)}</div></details></div>
            <div className="space-y-3">{visibleCandidates.map((candidate) => <CandidateCard key={candidate.id} candidate={candidate} busy={busyCandidate === candidate.id} canonicalizationEnabled={canonicalizationEnabled} onDecision={(decision, reason) => void act(candidate.id, { action: "review", decision, reason })} onPromote={(targetType, statement, reason, relatedElement) => void act(candidate.id, { action: "promote", targetType, statement, reason, relatedElementId: relatedElement?.id, elementKey: relatedElement?.key, evidenceSourceType: "conversation" })} onCanonicalize={async (payload, requestKey) => { const data = await act(candidate.id, { action: "canonicalize", ...payload }, requestKey); if (!data) return false; setSuccess(`Zeya’s canonical Representation has been updated to Version ${String(data.canonicalVersionNumber)}${data.idempotent === true ? " (confirmed replay)" : ""}.`); return true; }} />)}{visibleCandidates.length === 0 && <p className="py-8 text-center text-sm text-zeya-hush/45">Nothing matches these review filters.</p>}</div>
          </div>}
        </div>
      )}
    </section>
...
function CandidateCard({ candidate, busy, canonicalizationEnabled, onDecision, onPromote, onCanonicalize }: { candidate: ReviewConversation["candidates"][number]; busy: boolean; canonicalizationEnabled: boolean; onDecision: (decision: Exclude<ReviewDecision, "accepted_for_promotion">, reason?: string) => void; onPromote: (target: PromotionTarget, statement: string, reason: string | undefined, relatedElement?: { id: string; key: string }) => void; onCanonicalize: (payload: Record<string, unknown>, requestKey: string) => Promise<boolean> }) {
  ...
  const [confidenceScore, setConfidenceScore] = useState(Math.round(candidate.confidence * 100));
  const [canonicalRequestKey, setCanonicalRequestKey] = useState<string | null>(null);
  const evidenceEligible = candidate.trustLevel === "provider_attested" && !["zeya", "veya", "unknown"].includes(candidate.speakerRole);
  const targets = allowedPromotionTargets(candidate.candidateType).filter((target) => (target !== "evidence" || evidenceEligible) && !(canonicalizationEnabled && target === "representation_proposal"));
  const state = effectiveReviewState(candidate);
  const terminal = !["pending_review", "deferred"].includes(state);
  const invalidateCanonicalRequest = () => setCanonicalRequestKey(null);
  const relatedElement = candidate.relatedElements.find((element) => element.id === relatedElementId);
  const canTeach = Boolean(relatedElement && statement.trim() && reason.trim() && approvalReason.trim() && Number.isInteger(confidenceScore) && confidenceScore >= 0 && confidenceScore <= 100);
  return <article className="rounded-xl border border-zeya-graphite/25 p-4"><div className="flex flex-wrap justify-between gap-2"><span className="text-xs uppercase tracking-wider text-zeya-champagne/65">{candidate.candidateType.replaceAll("_", " ")}</span><span className="text-xs text-zeya-hush/45">{Math.round(candidate.confidence * 100)}% · {state.replaceAll("_", " ")}</span></div><p className="mt-3 text-sm leading-relaxed text-zeya-ivory/82">{String(candidate.content.summary ?? "Candidate intelligence")}</p><p className="mt-2 text-xs text-zeya-hush/48">{candidate.rationale}</p><p className="mt-2 text-[0.65rem] text-zeya-hush/40">Said by {candidate.speakerRole} · {trustLabel[candidate.trustLevel]} · turn {(candidate.sourceReference.turnIndexes?.[0] ?? 0) + 1}</p>
    {!terminal && <><label className="mt-4 block text-xs text-zeya-hush/60">{canonicalizationEnabled ? "Founder reason" : "Optional review reason"}<input value={reason} onChange={(e) => { setReason(e.target.value); invalidateCanonicalRequest(); }} maxLength={2000} className="mt-2 w-full rounded-lg bg-zeya-void/35 p-2 text-sm text-zeya-ivory/80" /></label><div className="mt-3 flex flex-wrap gap-2">{decisions.map(({ value, label }) => <button disabled={busy} key={value} onClick={() => onDecision(value, reason.trim() || undefined)} className="rounded-lg border border-zeya-graphite/35 px-2.5 py-1.5 text-xs text-zeya-hush/65 disabled:opacity-40">{label}</button>)}{targets.map((target) => <button disabled={busy || (target === "representation_proposal" && candidate.relatedElements.length === 0)} key={target} onClick={() => setPromoting(target)} className="rounded-lg border border-zeya-champagne/25 bg-zeya-champagne/5 px-2.5 py-1.5 text-xs text-zeya-champagne/75 disabled:opacity-40">Promote to {target.replaceAll("_", " ")}</button>)}{canonicalizationEnabled && allowedPromotionTargets(candidate.candidateType).includes("representation_proposal") && <button disabled={busy || candidate.relatedElements.length === 0} onClick={() => setTeaching(true)} className="rounded-lg border border-zeya-champagne/35 bg-zeya-champagne/10 px-2.5 py-1.5 text-xs text-zeya-champagne disabled:opacity-40">Approve and teach Zeya</button>}</div>{candidate.candidateType === "candidate_evidence" && !evidenceEligible && <p className="mt-2 text-[0.65rem] text-zeya-hush/45">Evidence requires a provider-attested human transcript source.</p>}</>}
    ...
    {teaching && <div className="mt-4 rounded-lg border border-zeya-champagne/30 p-3"><label className="text-xs text-zeya-hush/60">Confirmed canonical wording<textarea value={statement} onChange={(e) => { setStatement(e.target.value); invalidateCanonicalRequest(); }} className="mt-2 min-h-20 w-full rounded-lg bg-zeya-void/35 p-3 text-sm text-zeya-ivory/80" /></label><label className="mt-3 block text-xs text-zeya-hush/60">Related Representation Element<select value={relatedElementId} onChange={(e) => { setRelatedElementId(e.target.value); invalidateCanonicalRequest(); }} className="mt-2 w-full rounded-lg bg-zeya-aubergine p-2 text-zeya-ivory/80">{candidate.relatedElements.map((element) => <option key={element.id} value={element.id}>{element.key}</option>)}</select></label><label className="mt-3 block text-xs text-zeya-hush/60">Approval reason<input value={approvalReason} onChange={(e) => { setApprovalReason(e.target.value); invalidateCanonicalRequest(); }} maxLength={2000} className="mt-2 w-full rounded-lg bg-zeya-void/35 p-2 text-sm text-zeya-ivory/80" /></label><label className="mt-3 block text-xs text-zeya-hush/60">Confidence score (0–100)<input type="number" min={0} max={100} value={confidenceScore} onChange={(e) => { setConfidenceScore(Number(e.target.value)); invalidateCanonicalRequest(); }} className="mt-2 w-full rounded-lg bg-zeya-void/35 p-2 text-sm text-zeya-ivory/80" /></label><p className="mt-2 text-[0.65rem] text-zeya-hush/45">This founder approval atomically updates Zeya’s canonical Representation.</p><div className="mt-3 flex gap-2"><button disabled={!canTeach || busy} onClick={async () => { if (!relatedElement) return; const key = canonicalRequestKey ?? crypto.randomUUID(); if (!canonicalRequestKey) setCanonicalRequestKey(key); const ok = await onCanonicalize({ statement: statement.trim(), reason: reason.trim(), approvalReason: approvalReason.trim(), relatedElementId: relatedElement.id, elementKey: relatedElement.key, elementValues: { [relatedElement.key]: { value: statement.trim() } }, overallConfidenceScore: confidenceScore }, key); if (ok) { setCanonicalRequestKey(null); setTeaching(false); } }} className="rounded-lg bg-zeya-champagne/15 px-3 py-2 text-xs text-zeya-champagne disabled:opacity-40">Approve and teach Zeya</button><button onClick={() => setTeaching(false)} className="px-3 py-2 text-xs text-zeya-hush/55">Cancel</button></div></div>}
  </article>;
}
